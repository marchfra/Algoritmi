...
int main() {
	...
	out << "t,N,N_norm,lambda" << endl;

	while (t < 1000 && N > 0) {
		out << t << "," << N << "," << (double)N / N0 << "," << lambda << endl;
		N -= rad_decay(N, lambda);
		t += dt;
	}
	out << t << "," << N << "," << (double)N / N0 << ","<< lambda << endl;
...
int rad_decay(int N, double lambda) {
	int decayed = 0;
	for (int i = 0; i < N; i++) {
		if (drand48() < lambda) decayed++;
	}
	return decayed;
}
