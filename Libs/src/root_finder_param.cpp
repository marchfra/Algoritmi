...
int findRoots(double (*f)(const double &x, const double &k),
              double (*dfdx)(const double &x, const double &k), const double &k,
	...
	bracket(f, k, xa, xb, xL, xR, N, nRoots);
	...
	for (int i = 0; i < nRoots; i++) {
		if (method == "bisection") bisection(f, k, xL[i], xR[i], tol, roots[i]);
		else if (method == "falsePosition")
			falsePosition(f, k, xL[i], xR[i], tol, roots[i]);
		else if (method == "secant") secant(f, k, xL[i], xR[i], tol, roots[i]);
		else if (method == "newton")
			newton(f, dfdx, k, xL[i], xR[i], tol, roots[i]);
...
int findRoots(double (*f)(const double &x, const double &k), const double &k,
              const double &xa, const double &xb, const double &tol,
              double roots[], int &nRoots, const int N,
              const std::string method) {
	if (method == "newton")
	...
	return findRoots(f, nullptr, k, xa, xb, tol, roots, nRoots, N, method);
}

void bracket(double (*f)(const double &x, const double &k), const double &k,
...
int bisection(double (*f)(const double &x, const double &k), const double &k,
...
int bisection(double (*f)(const double &x, const double &k), const double &k,
	...
	int n;
	return bisection(f, k, xa, xb, xtol, -1.0, root, n);
}

int bisection(double (*f)(const double &x, const double &k), const double &k,
              double xa, double xb, const double &xtol, double &root,
              int &ntry) {
	return bisection(f, k, xa, xb, xtol, -1.0, root, ntry);
}
...
int bisection(double (*f)(const double &x, const double &k), const double &k,
              ...
              double &root) {
	int n;
	return bisection(f, k, xa, xb, xtol, ftol, root, n);
}
...
int falsePosition(double (*f)(const double &x, const double &k),
                  const double &k, double xa, double xb, const double &xtol,
...
int falsePosition(double (*f)(const double &x, const double &k),
                  ...
                  double &root) {
	int n;
	return falsePosition(f, k, xa, xb, xtol, -1.0, root, n);
}

int falsePosition(double (*f)(const double &x, const double &k),
                  const double &k, double xa, double xb, const double &xtol,
                  double &root, int &ntry) {
	return falsePosition(f, k, xa, xb, xtol, -1.0, root, ntry);
}
...
int falsePosition(double (*f)(const double &x, const double &k),
	...
	int n;
	return falsePosition(f, k, xa, xb, xtol, ftol, root, n);
}
...
int secant(double (*f)(const double &x, const double &k), const double &k,
           double xa, double xb, const double &xtol, const double &ftol,
           double &root, int &ntry) {
	int max_ntry = 64;
	double fa    = f(xa, k);
...
int secant(double (*f)(const double &x, const double &k), const double &k,
           double xa, double xb, const double &xtol, double &root) {
	int n;
	return secant(f, k, xa, xb, xtol, -1.0, root, n);
}

int secant(double (*f)(const double &x, const double &k), const double &k,
           double xa, double xb, const double &xtol, double &root, int &ntry) {
	return secant(f, k, xa, xb, xtol, -1.0, root, ntry);
}
...
int secant(double (*f)(const double &x, const double &k), const double &k,
           ...
           double &root) {
	int n;
	return secant(f, k, xa, xb, xtol, ftol, root, n);
}
...
int newton(double (*f)(const double &x, const double &k),
           double (*dfdx)(const double &x, const double &k), const double &k,
           double xa, double xb, const double &xtol, const double &ftol,
...
int newton(double (*f)(const double &x, const double &k),
	...
	int n;
	double dftol = 1.0e-3;
	return newton(f, dfdx, k, xa, xb, xtol, -1.0, dftol, root, n);
}
...
int newton(double (*f)(const double &x, const double &k),
           ...
           double xa, double xb, const double &xtol, const double &dftol,
           double &root, int &ntry) {
	return newton(f, dfdx, k, xa, xb, xtol, -1.0, dftol, root, ntry);
}
...
int newton(double (*f)(const double &x, const double &k),
	...
	int n;
	return newton(f, dfdx, k, xa, xb, xtol, ftol, dftol, root, n);
}
