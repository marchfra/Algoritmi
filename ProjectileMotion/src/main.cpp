...
int main() {
	...
	// Initial angle
	const double theta0    = 0.65;  //!< Min guess for launch angle
	const double theta_max = 0.92;  //!< Max value for theta
	const int nTheta       = 32;    //!< Number of explored thetas
	const double dTheta = (theta_max - theta0) / nTheta;  //!< Theta step size
	...
	try {
		...
		gTheta = theta0;                   // Set theta to min guess
	...
	try {
		...
		out << "t1,theta" << endl;
		gTheta = theta0;  // Initialise theta
	...
	try {
		...
		findRoots(Residual, theta_low, theta_upp, eps_theta, theta_star,
		          nTheta_star);
...
double Residual(const double& theta) {
	...
	const double t1     = roots[0];
	const double y_star = y_t(t1, theta);
